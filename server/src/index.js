...
import {
  elasticsearchSearch,
} from "./routes/elasticsearch.js";

import { bm25Search } from "./routes/bm25.js";
import { hybridSearch } from "./routes/hybrid.js";
dotenv.config();
...
const QUESTIONS = [
  ...
  // Short / autocomplete-oriented queries
  "What is a good book?",
...
async function embedText(text) {
  ...
  if (!Array.isArray(embedding)) {
    throw new Error(
      "OpenRouter response does not contain an embedding"
    );
  }

  if (embedding.length !== VECTOR_SIZE) {
    throw new Error(
      `Embedding dimension mismatch: expected ${VECTOR_SIZE}, got ${embedding.length}`
    );
  }
...
app.get(
  ...
  async (request, reply) => {
    ...
    if (mode === "bm25") {
      ...
      suggestions =
        await elasticsearchSearch(q, 5);
    } else if (mode === "hybrid") {
      suggestions = await hybridSearch(
        qdrant,
        COLLECTION,
        embedText,
        q,
